...
class TestMultiInvoiceDocument(unittest.TestCase):
    ...
    def test_einvoice_signature_page_attaches_despite_party_noise(self):
        sig = (
            "Generated By : 32AGJPP7919A1ZI\n"
            "Print Date: 07/07/2026 03:45:02\n"
        ...
        page1_data = {
            "data": {"invoice_summary": {
                "invoice_no": "CR-2403",
                "customer": "BENZIGAR HOSPITAL",
                "vendor": "ARUN PHARMACEUTICALS",
            }}
        }
        page4_data = {
            "data": {"invoice_summary": {
                "customer": "UNKNOWN HOSPITAL",
                "vendor": "32AGJPP7919A1ZI",
            }}
        }
        self.assertTrue(app._detect_invoice_continuation_page(
            3, None, sig, "CR-2403", INV4567_PAGE1,
            page_full_data=page4_data,
            current_full_data=page1_data,
        ))

    def test_nic_irp_pages_without_invoice_no_stay_one_group(self):
        page1 = (
            "1.e-Invoice Details Document No. : CR-2403 "
            "4A.Details of Goods / Services "
            "Sr Item HSN Quantity Unit Unit Price Discount Taxable "
            "1 DOIL 300490 30 UNT 101.71 91.54 3051.3 5+0|0+0 0 3107.75"
        )
        page2 = (
            "Sr Item HSN Quantity Unit Unit Price Discount Taxable "
        ...
        self.assertTrue(app._page_looks_like_nic_irp_item_continuation(page2))
        self.assertTrue(app._should_attach_page_to_current_invoice_group(
            1, None, page2, "CR-2403", page1,
            page_full_data=page2_data,
            current_full_data=page1_data,
        ))
        groups = _group_pages([
            ("CR-2403", page1, page1_data),
            (None, page2, page2_data),
            (None, page3, None),
            (None, page4, None),
        ])
        self.assertEqual(len(groups), 1)
        self.assertEqual(groups[0]["invoice_no"], "CR-2403")
        self.assertEqual(groups[0]["pages"], [0, 1, 2, 3])
        items = app._nic_irp_line_items_from_combined_ocr(groups[0]["ocr_text"])
        vault = next(i for i in items if "VAULT" in i["product_description"].upper())
        panto = next(i for i in items if "PANTODAC" in i["product_description"].upper())
        dex = next(i for i in items if "DEXONA" in i["product_description"].upper())
        ...
        self.assertEqual(str(dex["quantity"]), "100")
        self.assertEqual(dex["unit_price"], "8.28")
